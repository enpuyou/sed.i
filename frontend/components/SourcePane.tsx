...
export default function SourcePane({
  ...
  const [selectedArticle, setSelectedArticle] = useState<ContentItem | null>(
    null,
  );
  ...
  const openArticle = (article: ContentItem) => {
    ...
    setTabHistory((prev) => {
      ...
      return [
        { id: article.id, title: article.title || article.original_url },
        ...filtered,
      ].slice(0, 3);
    });
  };
...
function ArticleWithHighlights({
  ...
  return <div className="source-reader-body source-prose">{segments}</div>;
}

...
(function () {
  ...
  shadowStyle.textContent = `
    ...
    #reader-content img { max-width:100%;height:auto;border-radius:4px;display:block;cursor:zoom-in; }
  ...
  try {
    ...
    console.log('[sedi][reader-overlay][safari-resources] galleries=%d', gEls.length, gEls[0] ? gEls[0].innerHTML.slice(0,200) : '');
    if (gEls.length) {
      const gallery = gEls[0];
      const cs = getComputedStyle(gallery);
      console.log('[sedi][reader-overlay][safari-resources] gallery computed display=%s, grid-template-columns=%s, width=%s', cs.display, cs.gridTemplateColumns, cs.width);
      console.log('[sedi][reader-overlay][safari-resources] gallery children count=%d', gallery.children.length);
      Array.from(gallery.children).slice(0,6).forEach((c,i) => console.log('[sedi][reader-overlay][safari-resources] child[%d]=%s width=%s tag=%s', i, c.className || '-', getComputedStyle(c).width, c.tagName));
    }
  } catch (e) { console.log('[sedi][reader-overlay][safari-resources] debug error', e); }

  // ── Image zoom modal ──────────────────────────────────────────────────────
  // Appended to document.body so position:fixed is viewport-relative.
  ...
  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      try {
        const gEls = shadow.querySelectorAll('.sedi-gallery');
        console.log('[sedi][reader-overlay][safari-resources][postattach] galleries=%d', gEls.length);
        if (gEls.length) {
          const gallery = gEls[0];
          const cs = getComputedStyle(gallery);
          console.log('[sedi][reader-overlay][safari-resources][postattach] gallery computed display=%s, grid-template-columns=%s, width=%s', cs.display, cs.gridTemplateColumns, cs.width);
          console.log('[sedi][reader-overlay][safari-resources][postattach] gallery children count=%d', gallery.children.length);
            Array.from(gallery.children).slice(0,6).forEach((c,i) => {
              console.log('[sedi][reader-overlay][safari-resources][postattach] child[%d]=%s width=%s tag=%s', i, c.className || '-', getComputedStyle(c).width, c.tagName);
              const img = c.querySelector('img');
              if (img) {
                const ics = getComputedStyle(img);
                console.log('[sedi][reader-overlay][safari-resources][postattach] child[%d] img src=%s computed display=%s width=%s height=%s natural=%dx%d', i, img.currentSrc || img.src, ics.display, ics.width, ics.height, img.naturalWidth || 0, img.naturalHeight || 0);
              } else {
                console.log('[sedi][reader-overlay][safari-resources][postattach] child[%d] has no img', i);
              }
            });
          ...
          if (innerGrid) {
            ...
            console.log('[sedi][reader-overlay][safari-resources][postattach] innerGrid display=%s flexDirection=%s flexWrap=%s children=%d', igcs.display, igcs.flexDirection, igcs.flexWrap, innerGrid.children.length);
          }
        }
      } catch (e) { console.log('[sedi][reader-overlay][safari-resources][postattach] debug error', e); }
    });
  });

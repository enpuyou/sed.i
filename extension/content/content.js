...
async function extractAndInlineContent() {
  ...
  if (cloneBody) {
    // 0. For known article containers, hoist the article element to be a direct
    //    child of body and remove everything else. This prevents Readability from
    //    picking a low-quality candidate that loses headings/structure.
  ...
  console.log('[sedi][content.js][extension] loaded patch: gallery grouping rules updated');
  ...
  // 1. Class-based: sites that declare subtitle explicitly
  ...
  // 2. Content-based: the og:description often IS the subtitle text (Substack, Medium, etc).
  //    Remove any leading h2/h3/p whose text overlaps significantly with og:description.
  //    Uses the same prefix-match heuristic as the description-in-paragraph removal below.
  if (pageMeta.description) {
    const descNormSub = pageMeta.description.trim().toLowerCase();
    // Check the first few block elements (title area, before first h2 with real content)
    const candidates = Array.from(container.querySelectorAll('h2, h3, p')).slice(0, 5);
    for (const el of candidates) {
      const text = el.textContent.trim().toLowerCase();
      if (!text || text.length > 300) continue;
      // Only remove if the element IS the subtitle — not just shares a prefix.
      // Exact match, or the description starts with the element text (truncated subtitle),
      // or the element text starts with the description (description is the truncated form).
      // Require at least 20 chars to avoid false positives on short common phrases.
      const minLen = 20;
  ...
  if (window.__SEDI_SKIP_IMAGE_INLINE) {
    ...
    images.forEach((img) => {
      // Live DOM property: fully resolved, reflects lazy-load JS updates
          ...
          if (run.length === 0) run.push(node);
            ...
            if (lastGid === gid) run.push(node);
              ...
              if (run.length >= 2) {
                ...
                try { console.log('[sedi][content.js][extension] created contiguous gallery preview=', gallery.outerHTML.slice(0,400)); } catch (e) {}
              }
              run = [node];
            }
          }
